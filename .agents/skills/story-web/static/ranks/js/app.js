document.addEventListener('DOMContentLoaded', () => {
    ...
    // ========== Download Book ==========
    window.downloadBook = function(url, title, author, cover) {
        const match = url.match(/\/page\/(\d+)/);
        if (!match) { showToast('无法提取 book_id'); return; }
        ...
        showToast('开始下载: ' + title + ' (ID: ' + bookId + ')');

        fetch('/api/download', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({book_id: bookId, title: title, author: author, cover: cover})
        })
        .then(r => r.json())
        .then(data => {
            if (data.error) {
            ...
            showToast('下载任务已创建 (Job #' + data.job_id + ')');
            pollDownloadStatus(bookId, title, author, cover, btn, origText);
        })
        .catch(e => {
            showToast('下载失败: ' + e);
            btn.textContent = origText;
    ...
    function pollDownloadStatus(bookId, title, author, cover, btn, origText) {
        let attempts = 0;
        const timer = setInterval(() => {
            ...
            .then(data => {
                ...
                const job = items.find(i => i.book_id === bookId);
                if (job && job.state === 'done') {
                    clearInterval(timer);
                    btn.textContent = '归档中...';
                    showToast('下载完成，正在归档...');
                    archiveBook(bookId, title, author, cover, btn, origText);
                } else if (job && job.state === 'failed') {
                    clearInterval(timer);
                    btn.textContent = origText;
                    btn.disabled = false;
                    btn.style.opacity = '1';
                    showToast('下载失败: ' + (job.message || '未知错误'));
                }
            });
            if (attempts > 60) {
                clearInterval(timer);
    ...
    function archiveBook(bookId, title, author, cover, btn, origText) {
        fetch('/api/download/archive', {
            method: 'POST',
            headers: {'Content-Type': 'application/json'},
            body: JSON.stringify({book_id: bookId, title: title, author: author, cover: cover})
        })
        ...
        .then(data => {
            if (data.moved && data.moved.length > 0) {
                ...
                showToast('归档完成: ' + data.target);
            } else {
                btn.textContent = origText;
                btn.disabled = false;
                btn.style.opacity = '1';
                showToast('归档失败: 没有找到下载文件');
            }
        });
    }
    ...
    function getFilteredBooks(cat) {
        ...
        if (filter === 'new') {
            ...
            books = books.filter(b => b.status === '完结');
        } else if (filter === 'ongoing') {
            books = books.filter(b => b.status === '连载中');
        }
        ...
        if (sort === 'reads') {
            ...
            books.sort((a, b) => {
                const ac = parseInt(a.chapters) || 0;
                const bc = parseInt(b.chapters) || 0;
                return bc - ac;
            });
        }
        // sort === 'rank' 保持原序
            ...
            card.innerHTML = `
                ...
                <div class="book-info">
                    ...
                    <div class="book-meta">
                        ...
                        ${book.status ? `<span class="book-status ${book.status === '完结' ? 'done' : 'ongoing'}">${escapeHtml(book.status)}</span>` : ''}
                        ${book.chapters ? `<span class="book-chapters">${escapeHtml(book.chapters)}章</span>` : ''}
                    </div>
                    <p class="book-intro">${escapeHtml(book.intro).replace(/\n/g, '<br>')}</p>
                    <div class="book-actions">
                        ${book.url ? `<a class="book-read-btn" href="${escapeAttr(book.url)}" target="_blank" rel="noopener" onclick="event.stopPropagation()">阅读</a>` : ''}
                        <button class="book-download-btn" type="button" onclick="event.stopPropagation();downloadBook('${escapeAttr(book.url || '')}', '${escapeAttr(book.title)}', '${escapeAttr(book.author)}', '${escapeAttr(book.cover || '')}')">下载</button>
