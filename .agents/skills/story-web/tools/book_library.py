...
def extract_concept_info(concept_path):
    """从concept.md提取书名、简介、题材等信息"""
    result = {
        'book_name': '',
        'synopsis': '',
        'genre': '',
    ...
    try:
        ...
        if intro_match:
            result['synopsis'] = intro_match.group(1).strip()[:200]
        ...
        for sub in rw_dir.iterdir():
            if sub.is_dir():
                ...
                if ch_dir.is_dir():
                    ...
                    if chapters:
                        # 从concept.md提取信息
                        concept_path = sub / "concept.md"
                        concept_info = extract_concept_info(concept_path) if concept_path.exists() else {}
                        
                        # 计算进度
                        total_chapters = len(chapters)
                        progress = f"{total_chapters}章"
                        
                        versions.append({
                            "type": "rewrite",
                            "name": sub.name,
                            "chapters": chapters,
                            "book_name": concept_info.get('book_name', sub.name),
                            "synopsis": concept_info.get('synopsis', ''),
                            "genre": concept_info.get('genre', ''),
                            "tags": concept_info.get('tags', []),
                            "status": concept_info.get('status', '进行中'),
                            "progress": progress,
                            "has_export": (sub / "export").exists(),
                        })
...
def format_book_list(books, verbose=False):
    ...
    for i, book in enumerate(books, 1):
        ...
        if verbose:
            ...
            # 显示仿写版本信息
            if book.get('versions'):
                for v in book['versions']:
                    if v.get('type') == 'rewrite':
                        lines.append(f"   仿写: {v.get('name', '')} ({v.get('progress', '')})")
