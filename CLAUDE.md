...
Phase 0.75:拆书 (story-dissect)      → _cache/source_analysis/（全本分析，供开书消费）
Phase 1:   开书 (pro, 1 call)        → concept.md + settings/（设定+角色名+行为模式+弧线）
Phase 2:   guides (flash, N 并行)     → plot_{N}.md + style_{N}.md（节拍映射+冲突替换+高光）
...
# 分步（dissect → open-book 自动衔接）
python tools/rewrite_chapters.py --config configs/xxx.json --phase dissect
...
# 注意：dissect 只分析源文，产出存在 _cache/source_analysis/
# open-book 自动检测 dissect 产出并复用，跳过自身的曲线分析
# dissect → open-book → guides → write 完整链
python tools/pipeline.py --config configs/xxx.json --phase open-book,guides,write
# （open-book 会自动触发 prep，dissect 手动跑）
