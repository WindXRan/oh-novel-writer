...
3. **检查核心 Agent 部署状态**（只检查项目内 agents，不要假设一定存在）：
   ...
   - full 可选：`.claude/agents/plagiarism-checker.md`（仿写项目必检，普通项目跳过）
...
**Agent 5: plagiarism-checker**（subagent_type: plagiarism-checker）
- full 模式调用。lean/solo 模式跳过。
- 审查视角：五层抄袭检测（结构层、语句层、人设层、情节装置层、句式层）。
- 适用场景：
  - `/story-rewrite` 试水后的自检（对比源文本）
  - 任何新文本的原创性审查（无源文本时做独立检测）
  - 收到抄袭投诉时的取证对比
- 提示指令：
  ```
  你是 plagiarism-checker，在五个层面检测文本间的结构性相似度。
  你的任务是【找抄袭证据】，不做创作评判，不评价文学质量。
  项目路径：{项目根}
  新文本：{文件路径/章节/必要摘录}
  源文本：{源文本路径（如有，必须提供）/ 或写"无源文本，做独立检测"}
  审查基准包摘要：{Phase 1 形成的 rubric / fallback 摘要，必须内联}
  Rubric Source: file | embedded fallback
  可选补充参考：如项目已部署 story-setup reference bundle，可读取 `story-setup/references/agent-references/quality-checklist.md`；若不可读，不影响检测。
  检查项：
  1. 结构层：叙事功能序列、转折时机、场景切换顺序是否完全对应？
  2. 语句层：是否存在完全照搬、直接换词、句式复制？
  3. 人设层：角色是否存在1:1功能映射？
  4. 情节装置层：核心情节装置、关键台词、信物道具是否复用？
  5. 句式层：段落结构、感官描写顺序、心理描写公式是否一致？

  输出格式：
  VERDICT: CLEAN / SUSPICIOUS / PLAGIARIZED
  RISK_LEVEL: S1 | S2 | S3 | S4
  FINDINGS: 必须使用统一 Findings Schema，severity 必须是 S1/S2/S3/S4；category 使用 plagiarism。
  EVIDENCE: [五层检测证据清单，按严重度排列]
  ```
...
- plagiarism-checker: CLEAN / SUSPICIOUS(n) / PLAGIARIZED / NOT_RUN
