...
def main():
    ...
    original_chars = len(re.sub(r'\s', '', text))
    
    result = post_process(text, aggressive)
    processed_chars = len(re.sub(r'\s', '', result))
    ...
    print(f'Change: {processed_chars - original_chars} chars ({(processed_chars - original_chars) / original_chars * 100:.1f}%)')
