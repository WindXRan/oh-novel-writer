...
| 续写/日更 | 续写、继续写、继续、日更、接着写、往下写 | 见下方「续写路由」 |
...
用户说「续写」「继续写」「继续」「日更」时，按以下逻辑路由：

1. **确定目标项目**：扫描项目根目录，找到含 `追踪/` 或 `设定/` 的书名目录（优先读 `.active-book`；多本时列出让用户选）
2. **检测仿写项目**：检查项目根目录是否存在 `仿写框架.md`
   - **存在** → 仿写项目，路由到 `/story-rewrite`（断点续写模式：扫描正文/找到最大章节编号 N，从第 N+1 章续写）
   - **不存在** → 常规项目，路由到 `/story-long-write`（走 workflow-daily.md 的 Step 1-4 日更流程）
3. **正文为空时**：如果 `正文/` 目录为空且存在 `大纲/章纲_全书.md`，从第 1 章开始写；如果章纲也不存在，先走章纲生成流程

> 仿写项目的断点续写流程详见 `story-rewrite` SKILL.md 的「断点续写（从中断处继续）」章节。
