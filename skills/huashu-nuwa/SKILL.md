...
**⚠️ 仅在目标为网文作者时执行。通用人物 Skill 跳过此步。**

从该作者的多部作品中提取书名和简介的商业模式，写入 `references/research/07-commercial.md`。

**书名命名模式**：

| 维度 | 提取内容 |
|------|---------|
| 结构模板 | 如「XX的XX」「XX：XX」「动词+名词」等高频结构 |
| 字数范围 | 最短/最长/最常见字数 |
| 情绪标签 | 悬念型/甜宠型/反差型/古风型/直白型 |
| 关键词池 | 高频出现的意象/身份/动作词（如「总裁」「闪婚」「重生后」） |
| 反差手法 | 是否常用身份反差/时空反差/认知反差 |

提取方法：列出该作者所有作品书名 → 归类结构 → 提炼共性 → 统计频率

**简介钩子手法**：

| 维度 | 提取内容 |
|------|---------|
| 开篇钩子类型 | 对话体/悬念铺垫/设定甩出/反差场景/问题抛出 |
| 结构模板 | 几段式？先人设后冲突？先冲突后人设？ |
| 信息密度 | 每句话传递几个信息点 |
| 收尾方式 | 留悬念/抛问题/反转/直接亮CP |
| 情绪基调 | 沙雕/虐心/爽文/悬念/甜 |
| 禁忌 | 不做什么（如不过度剧透、不用第三人称叙述等） |

提取方法：列出该作者所有作品简介 → 逐篇分析钩子/结构/收尾 → 归类共性模式

**输出格式**（写入 `07-commercial.md`）：
