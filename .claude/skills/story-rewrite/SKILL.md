...
|------|------|------|------|
| `--mode=style` | 文风仿写 | 中 | 高 |
| `--mode=structure` | 结构仿写 | 中 | 高 |
| `--mode=both` | 文风+结构 | 中 | 最高 |
| `--mode=speed` | 超快模式 | 最快 | 中 |
...
| 参数 | 值 | 说明 |
|------|-----|------|
| `--chunk-size` | 30 | 每区间 30 章 |
| `--parallel` | 30 | 并行 30 agent |
| `--distill-interval` | 3 | 每 3 个区间蒸馏一次 |

### 优化项

| 标准模式 | 超快模式 |
|----------|----------|
| Layer A + B + C | 只用 Layer A（统计指纹） |
| 结构映射 | 跳过（style 模式） |
| 完整校验 | 轻量校验（字数+禁用词） |
| 每区间更新 truth files | 每 3 个区间更新 |
