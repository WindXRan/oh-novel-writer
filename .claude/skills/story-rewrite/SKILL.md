...
| `{书名}/简介.md` | 书名+简介+TIPS标签 | agent read，发布用 |
...
1. **源文本分析**：
   ...
   - 全书章数、卷结构、每卷核心事件
       - 提取3-5段语感样本 + 1章完整样本
    - 分析叙述者风格（是否跳评论、网感程度、句长、口语化程度、情绪外放程度）

   **核心原则：模仿 > 定义。** 给 agent 语感样本让它自然模仿，比告诉它"你是XX型人格"更有效。前者改变选词分布（更像人），后者只是表演风格（仍有AI规律）。不注入人格名，只注入语感样本。
...
2. **自动生成新书方案**（AI全自动，不询问用户）：
   ...
   ⚠️ **生成简介（必做）**：方案确定后，必须生成简介写入 `{书名}/简介.md`。
    - 主线程直接生成（无需 subagent）
    - 读取 style SKILL.md 的「书名与简介」部分，了解风格的开头策略、信息顺序、结尾钩子类型、字数范围、格式要求
    - 按风格规则生成简介，**格式严格遵循风格 SKILL.md 的规定**（每种风格的简介结构不同，不自创格式）
   - 字数控制遵循风格规定（空留风格80-250字，其他风格默认150-300字）
   - 生成后自检：是否遵守风格的开头策略？结尾是否有钩子？是否避免剧透？
