...
| 句长标准差 | 10句窗口内句长标准差 | >5字 | 3-5字 | <3字 |
| 动作-反应耦合率 | 带双重描写的动作数 / 总动作数 | ≤40% | 40-60% | >60% |
| 句首重复率 | 连续3句同主语开头的次数 / 总句数×0.3 | ≤2次/千字 | 3-5次/千字 | >5次/千字 |
| 段落均匀度 | 连续3段句数相同的次数 | ≤1次/千字 | 2-3次/千字 | >3次/千字 |

> 备注：核心场景（开篇、高潮、收束）出现 1 次重复描写即按 ≥1 档加权（轻→中，中→重）。
>
> 以上阈值为参考值，需结合题材特点调整。例如古风题材的对话标签密度天然偏高，应适当放宽。
>
> **综合判定规则**：取十项指标中的最高档位。任一指标达重度即按重度处理；无重度时，中度指标 ≥3 项按中度处理，否则按轻度处理。结构类指标（句长标准差、动作-反应耦合率、句首重复率、段落均匀度）任一达重度，强制按重度处理。
...
#### 门禁 G：深层结构去AI（AIGC检测器专项）

> 针对朱雀/图灵等AIGC检测器。这些检测器看统计特征，不只是禁用词。

**G1. 句长随机化**
- 每10句为窗口，句长标准差必须 > 5字
- 禁止连续3句都在10-20字区间
- 每10句内必须有至少1个超短句（≤5字）和至少1个长句（≥25字）
...
**G3. 动作-反应解耦**
- 动作后不补身体反应的比例 ≥ 40%
- 禁止连续3个动作都带"动作+即时反应"双重描写
- 情绪反应可以用沉默/空白/跳过代替

**G4. 信息密度对比**
- 每章至少1段"极简过渡"（≤2句，只交代事实）
- 禁止每个场景都完整展开（起因→经过→结果），允许场景中途切入
- 关键场景和过渡场景的段落句数比 ≥ 2:1

**G5. 句首分布随机化**
- 连续3句以同一主语开头 = AI特征
- 每10句内句首主语类型 ≥ 4种（人名/代词/物件/环境/对话/无主句）
- 鼓励用物件、环境、声音、对话开头
...
- 对话优化：{N} 处
- 结尾修正：{N} 处
- 结构去AI（Gate G）：{N} 处（句长随机化 {N}、段落打散 {N}、动作-反应解耦 {N}、信息密度 {N}、句首分散 {N}、留白跳跃 {N}、情绪去模板 {N}）
