...
去AI模块存放在 `de-ai-modules/` 目录下，每个模块是独立的 .md 文件：

| 模块文件 | 作用 |
|---------|------|
| `删连接词.md` | 去掉「然而」「但是」「因此」及单字连接词「但」「又」「却」「就」 |
| `拆长句.md` | >30字的句子拆成短句 |
| `降级副词.md` | 「微微」「轻轻」「缓缓」用具体动作替代 |
| `轻微倒装.md` | 语序微调，打破规律性 |
| `加微重复.md` | 模拟人类的重复习惯 |
| `加模糊词.md` | 精确数字改为模糊表达 |
| `降级连接词频率.md` | 降低连接词使用频率 |
| `字数模糊化.md` | 数字/时间/距离模糊化 |
| `污染AI流畅性.md` | 加入「卡顿感」 |
| `真实人类口误.md` | 加入微小错误（最后手段） |
| `标点后处理.md` | **必选，最后执行** |
| `得地的.md` | **必选，标点后处理之后执行** |
| `数字模糊化.md` | 精确数字改为模糊表达（阿拉伯数字场景） |
| `de-ai-connectors.py` | **自动化脚本**：连接词降频 + 得地的修正 |
...
连接词降频 + 得地的修正，使用 Python 脚本批量处理：

```bash
# 单章处理
python de-ai-modules/de-ai-connectors.py "正文/第001章_乱葬岗.md"

# 批量处理
for f in 正文/第*.md; do python de-ai-modules/de-ai-connectors.py "$f"; done

# 预览模式（不修改文件）
python de-ai-modules/de-ai-connectors.py "正文/第001章_乱葬岗.md" --dry-run
```
