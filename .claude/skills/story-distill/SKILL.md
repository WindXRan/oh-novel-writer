...
**等待用户确认后** → 进入 Phase 3.5。

---

## Phase 3.5：规则质量量化评分

**目的**：在进入 Phase 4 前，量化评估提取规则的质量，为 git ratchet 提供基线。

### 4 维度评分（总分 100）

| 维度 | 权重 | 评估方法 | 满分标准 |
|------|------|---------|---------|
| **规则具体性** | 30% | 每条规则是否有明确的 A2（触发场景）和 E（执行步骤） | ≥80% 规则有清晰 A2+E |
| **边界清晰度** | 25% | 每条规则是否有 B（边界与盲点） | ≥70% 规则有 B |
| **测试通过率** | 25% | Phase 6 压力测试的通过率 | ≥80% 通过 |
| **覆盖完整度** | 20% | 8 个维度是否都有输出 | 缺失维度 ≤1 |

### 执行步骤

1. 读取 `references/candidates/` 下所有规则
2. 统计各维度指标
3. 计算加权总分
4. 写入 `{作者名}/references/quality-score.json`
...
|------|------|------|
| ≥80 | 优秀 | 直接进入 Phase 4 |
| 60-79 | 合格 | 进入 Phase 4，标记薄弱维度 |
| <60 | 不合格 | 回炉 Phase 2 补充提取 |

**等待评分完成后** → 进入 Phase 3.6。

---

## Phase 3.6：框架级 Judge 评估

**目的**：从整体框架角度评估规则集合的协调性，弥补 Phase 3 规则级验证的不足。

**与 Phase 3 的区别**：
- Phase 3：规则级过滤（单条规则通过/不通过）
- Phase 3.6：框架级评估（规则集合是否协调、完整、无冲突）

### 5 维度评估（总分 100）

| 维度 | 权重 | 评估标准 |
|------|------|---------|
| **规则协调性** | 25% | 规则之间是否有矛盾？是否互相强化？ |
| **场景覆盖度** | 25% | 常见写作场景是否都有对应规则？ |
| **边界可操作性** | 20% | 边界条件是否清晰到可以执行？ |
| **信息密度** | 15% | 规则是否简洁而不遗漏关键信息？ |
| **诚实边界** | 15% | 是否明确标注了规则的局限性？ |

### 执行方式

启动独立 subagent（general 类型）：
```
你是独立的 judge，负责评估规则框架的整体质量。

读取以下文件：
- {作者名}/references/candidates/*.md（所有规则）
- {作者名}/references/constructed/*.md（构造后的规则）
...
- 总分 ≥80 → 进入 Phase 4
- 总分 60-79 → 标记问题，进入 Phase 4 时一并修复
- 总分 <60 → 回炉 Phase 2 补充提取
...
**机制**：
1. 回炉前，用 Phase 3.5 的 4 维度评分计算当前分数（old_score）
2. 执行回炉修改
3. 重新评分（new_score）
4. 比较：
   - new_score > old_score → 保留改动，继续下一轮
   - new_score ≤ old_score → revert 改动，停止迭代
5. 最多迭代 3 次（max_iterations = 3）

**迭代记录**：
```
轮次 | old_score | new_score | Δ | 动作
1    | 65        | 72        | +7 | keep
2    | 72        | 74        | +2 | keep
3    | 74        | 73        | -1 | revert, stop
```

**触顶条件**：
- 连续 2 轮 Δ < 2 分 → 触顶，提示用户
- 达到 max_iterations → 停止，进入 Phase 7
...
### 7.4 量化 Rubric 评分（5 维度，总分 100）

| 维度 | 权重 | 评估标准 |
|------|------|---------|
| **结构完整性** | 20% | 必要 section 齐全（frontmatter、心智模型、决策启发式、表达DNA、反模式、章纲模板） |
| **规则可执行性** | 25% | 每条规则有明确的 A2（触发场景）和 E（执行步骤），可直接用于写作 |
| **边界清晰度** | 20% | 每条规则有 B（边界与盲点），不会被误用 |
| **测试通过率** | 25% | Phase 6 压力测试通过率 |
| **表达辨识度** | 10% | SKILL.md 的语言风格是否能体现作者特色 |
...
```json
{
  ...
  "weak_dimensions": ["边界清晰度", "表达辨识度"]
}
```

**动作**：
- 总分 ≥80 → 注册到 story-style
- 总分 60-79 → 标记薄弱维度，用户决定是否注册
- 总分 <60 → 回炉 Phase 4
...
### 8.0 Git Ratchet 版本控制

**目的**：防止质量退化，每次蒸馏必须比上次更好或持平。

**机制**：
1. 蒸馏完成后，用 Phase 3.5 的 4 维度评分计算总分
2. 读取 `{作者名}/references/quality-score.json`（如存在）
3. 比较：新分 ≥ 旧分 → 保留；新分 < 旧分 → 警告并建议 revert
4. 写入 `quality-score.json`

**quality-score.json 格式**：
```json
{
  "version": 1,
  "score": 78,
...
**ratchet 规则**：
- 连续 2 轮 Δ < 2 分 → 触顶，提示用户是否继续
- 新分 < 旧分 → 自动警告，建议检查改动
- 所有改动在 git 分支上，用 revert 而非 reset
